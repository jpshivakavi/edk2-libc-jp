...
#ifdef UEFI_C_SOURCE
#  define INITFUNC PyInit_uefi
#  define MODNAME "uefi"
#  define MODNAME_OBJ &_Py_ID(uefi)  
...
#elif !defined(UEFI_C_SOURCE) && !defined(_MSC_VER) && (!defined(__WATCOMC__) || defined(__QNX__) || defined(__VXWORKS__))
extern char **environ;
#endif /* !_MSC_VER */
...
#if defined(HAVE_EXECV) || defined(HAVE_TRUNCATE)
static PyObject *
...
#if defined(HAVE_FSYNC) || defined(HAVE_FCHDIR) || defined(HAVE_FDATASYNC)
static PyObject *
...
_pystat_fromstructstat(PyObject *module, STRUCT_STAT *st)
{
...
#endif
#else /* UEFI_C_SOURCE */
    PyStructSequence_SET_ITEM(v, 1, PyLong_FromUnsignedLongLong(0));
    PyStructSequence_SET_ITEM(v, 2, _PyLong_FromDev(0));    
#endif

#ifndef UEFI_C_SOURCE        
    PyStructSequence_SET_ITEM(v, 3, PyLong_FromLong((long)st->st_nlink));
#else
    PyStructSequence_SET_ITEM(v, 3, PyLong_FromLong(0));
#endif
    ...
    PyStructSequence_SET_ITEM(v, 4, _PyLong_FromUid(0));
    PyStructSequence_SET_ITEM(v, 5, _PyLong_FromGid(0));        
#else
...
#else
    fill_time(module, v, 9, 12, 15, 0, 0);
#endif
...
_posix_listdir(path_t *path, PyObject *list)
{
    ...
    for (;;) {
        ...
        dm_name = (char*)ep->d_name;
        dm_name_size = NAMLEN(ep);
        ...
        dm_name_size = strlen(dm_name);
#endif
        
        if (dm_name[0] == '.' &&
            (dm_name_size == 1 ||
             (dm_name[1] == '.' && dm_name_size == 2)))
            continue;
        if (return_str)
           v = PyUnicode_DecodeFSDefaultAndSize(dm_name, dm_name_size);
        else
           v = PyBytes_FromStringAndSize(dm_name, dm_name_size);
        if (v == NULL) {
            Py_CLEAR(list);
            break;
        }
...
/*[clinic end generated code: output=ab7f1a88b654581c input=dc93b1d3984cffb6]*/
{
    ...
    buffer = (char*)PyMem_Malloc(sizeof(char) * (strlen(path->narrow) + 1));
    if (!buffer) {
        return NULL;
    }
    strcpy(buffer, path->narrow);
    for (char *p = strchr(buffer, '/'); p; p = strchr(p, '/')) {
        *p = '\\';
    }
    ...
    if (!end || end == buffer) {
      ...
      char last = *end;
      *end = 0;      
      PyObject *root = PyUnicode_DecodeFSDefault(buffer);
      *end = last;
      PyObject *tail = PyUnicode_DecodeFSDefault(end);
          
      result = Py_BuildValue("NN", root, tail);
    }
    PyMem_Free(buffer);
...
#endif
#ifdef UEFI_C_SOURCE
       /* UEFI rename() won't allow rename to already existing file */
    unlink(dst->narrow);
#endif          
...
#if defined(HAVE_FORK) || defined(HAVE_FORK1) || defined(HAVE_FORKPTY)
...
#if defined(TERMSIZE_USE_CONIO) || defined(TERMSIZE_USE_IOCTL) || defined(UEFI_C_SOURCE)
/*[clinic input]
...
/*[clinic end generated code: output=fbab93acef980508 input=ead5679b82ddb920]*/
{
...
#ifdef UEFI_C_SOURCE
    {
        ...
        if (fstat(fd, &s))
          return PyErr_SetFromErrno(PyExc_OSError);
        size.Offset = s.st_physsize;
        columns = size.XYpos.Column-1;
        lines = size.XYpos.Row-1;
    }    
#endif /* UEFI_C_SOURCE */
...
#elif defined(UEFI_C_SOURCE)        
        if (mode_bits == S_IFDIR)
          result = (self->d_type & DT_DIR) != 0;
        else
          result = (self->d_type & DT_REG) != 0;        
#else /* POSIX */
        if (mode_bits == S_IFDIR)
...
#if defined(UEFI_C_SOURCE)
PyDoc_STRVAR(os_fdstat__doc__,
"fdstat($module, /)\n"
"--\n"
"\n"
"UEFI LibC specific. Returns number of open and closed file descriptors");

#define OS_FDSTAT_METHODDEF \
    {"fdstat", _PyCFunction_CAST(os_fdstat), METH_FASTCALL, os_fdstat__doc__},


static PyObject *
os_fdstat(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
