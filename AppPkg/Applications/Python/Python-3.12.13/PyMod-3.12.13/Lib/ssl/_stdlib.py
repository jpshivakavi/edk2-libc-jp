...
_IntEnum._convert_(
    'AlertDescription', _SSL_PUBLIC,
    lambda name: name.startswith('ALERT_DESCRIPTION_'),
    source=_ssl)

_IntEnum._convert_(
    'SSLErrorNumber', _SSL_PUBLIC,
    lambda name: name.startswith('SSL_ERROR_'),
    source=_ssl)

_IntFlag._convert_(
    'VerifyFlags', _SSL_PUBLIC,
    lambda name: name.startswith('VERIFY_'),
    source=_ssl)

_IntEnum._convert_(
    'VerifyMode', _SSL_PUBLIC,
...
if os.name == 'uefi':
    # Do not import Lib/socket.py here — import ssl + socket stdlib breaks Shell
    # exit on VS2022 UEFI even when "import socket" alone passes (_ssl + socket.py).
    import _socket as _socket_builtin
    socket = _socket_builtin.socket
    SOCK_STREAM = _socket_builtin.SOCK_STREAM
    SOL_SOCKET = _socket_builtin.SOL_SOCKET
    SO_TYPE = _socket_builtin.SO_TYPE
    _GLOBAL_DEFAULT_TIMEOUT = None
    _socket_py = None

    def _lazy_socket_py():
        global _socket_py, create_connection, _GLOBAL_DEFAULT_TIMEOUT
        if _socket_py is None:
            import socket as _socket_py
            create_connection = _socket_py.create_connection
            _GLOBAL_DEFAULT_TIMEOUT = _socket_py._GLOBAL_DEFAULT_TIMEOUT
        return _socket_py

    def create_connection(*args, **kwargs):
        return _lazy_socket_py().create_connection(*args, **kwargs)

    class _SocketModuleProxy:
        __slots__ = ()
        def __getattr__(self, name):
            return getattr(_lazy_socket_py(), name)

    _socket = _SocketModuleProxy()
else:
...
def _materialize_purpose(purpose):
    """UEFI: Purpose enum values are OID strings until real SSL use (import ssl smoke)."""
    if os.name != 'uefi':
        return purpose
    if isinstance(purpose, _ASN1Object):
        return purpose
    if isinstance(purpose, Purpose):
        return _ASN1Object.__new__(_ASN1Object, purpose.value)
    return purpose


if os.name == 'uefi':
    class Purpose(_Enum):
        """SSLContext purpose flags (OID strings; OpenSSL lookup deferred on UEFI)."""
...
if os.name == 'uefi':
    ...
    class SSLSocket(_socket.socket if os.name != 'uefi' else object):
