...
33. **2026-09-09 — boot traces turned OFF at build time; the code stays. Both toolchains are now silent on a normal boot.** **`/DPY_UEFI_BOOT_TRACE=1`** removed from the MSFT **`CC_FLAGS`** of **both** **`Python312.inf`** and **`Python312_MIN.inf`**, each with a comment block above the line explaining how to put it back. Every trace site is **`#ifdef`**-guarded already (**`PY312_BOOT_PRINT`** in **`edk2main.c:25`**, **`PY312_CONSOLE_TRACE`** in **`edk2console.c:17`**, **`py312_boot_print_ascii`** in **`py312boot.h`**), so **no trace code was deleted** — only its define. **One site was not guarded and had to be fixed:** **`edk2main.c:60`**'s **`Print(L"Python312: UefiMain\r\n")`** was unconditional, which made it the *only* output a GCC image ever produced and would have left a "traces off" VS2022 image still printing one line; it is now **`#ifdef PY_UEFI_BOOT_TRACE`** too. **What deliberately stays unconditional:** the **`Failed to …`** error paths in **`UefiMain`** and the **CPU-fault report** in **`edk2excep.c:106`** — those are notices that something went *wrong*, not boot progress, and gating the fault report is exactly the defect §11.8 #4 was. **Cost, recorded in the docs rather than discovered later:** §1.1 of the smoke doc — the **`switched stack min_rsp=… limit=… size=…`** measurement — and the **`before py_install_idt`** confirmation both need a throwaway rebuild with the flag now, so **§1.1 became a diagnostic procedure instead of a per-build step**. **Upside beyond a quiet console:** the diagnostic asymmetry between the toolchains is gone, and by *removal* rather than by adding the flag to GCC — a stock image of either toolchain behaves and reports identically.
