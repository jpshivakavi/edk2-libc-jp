...
32. **2026-09-09 — ALL FOUR CONFIGURATIONS GREEN AT ONE CODE STATE, a first for this port. Tag `python312-seh-fix-all-configs-2026-09-09`.** VS2022 FULL rebuilt and swept (**§2/§3/§4** and **§5.8**), completing the matrix alongside **VS2022 MIN** (§7.1), **GCC MIN** (§7.2) and **GCC FULL** (§7.3) — [`Python312_Smoke_Tests.md`](./Python312_Smoke_Tests.md) **§7.4** carries the table. **Every previous sign-off covered FULL on both toolchains but MIN on neither**, and both MIN builds are shipped deliverables; that is the hole this closes. **For the `edk2_seh_*` fixes it means verification on every configuration they compile into**, with **§5.8 green on both FULL builds** — the *unhandled* reporting branch the fixes did not modify but sit adjacent to inside the same function. **Remaining known deviation is §11.8 #1 alone** (the GCC stack-alignment expression), which is cosmetic and cannot fault: **`malloc`** returns a 16-byte-aligned base and 512 is a multiple of 16, so **`base % 512`** is always a multiple of 16 and the GCC expression — despite aligning nothing — has never produced a misaligned **`rsp`**. Unifying it on the round-up form is strictly stronger and stays inside the **`malloc(size + 1024)`** slack, but it **moves the address GCC runs its stack on**, so it is a change plus a GCC re-test rather than a free cleanup. **Noted while analysing it, deliberately left alone:** **`stack_limit`** derives from the raw **`malloc`** base rather than **`aligned_stack`**, eroding up to 511 of the 8 192-byte margin — immaterial against 64 MB and the 39–71 KB actually used, and folding it in would shift **`PyOS_CheckStack()`**'s threshold, which does not belong in a cosmetic fix.
33. **2026-09-09 — boot traces turned OFF at build time; the code stays.** *(The claim of silence made in this entry was premature — hardware found a second ungated line and a DSC-level define still forcing the flag on. See item 35 for what was actually required and verified.)* **`/DPY_UEFI_BOOT_TRACE=1`** removed from the MSFT **`CC_FLAGS`** of **both** **`Python312.inf`** and **`Python312_MIN.inf`**, each with a comment block above the line explaining how to put it back. Every trace site is **`#ifdef`**-guarded already (**`PY312_BOOT_PRINT`** in **`edk2main.c:25`**, **`PY312_CONSOLE_TRACE`** in **`edk2console.c:17`**, **`py312_boot_print_ascii`** in **`py312boot.h`**), so **no trace code was deleted** — only its define. **One unguarded site was found and fixed here:** **`edk2main.c:60`**'s **`Print(L"Python312: UefiMain\r\n")`** was unconditional, which made it the *only* output a GCC image ever produced; it is now **`#ifdef PY_UEFI_BOOT_TRACE`** too. **There was a second, and this entry missed it** — see item 35. **What deliberately stays unconditional:** the **`Failed to …`** error paths in **`UefiMain`** and the **CPU-fault report** in **`edk2excep.c:106`** — those are notices that something went *wrong*, not boot progress, and gating the fault report is exactly the defect §11.8 #4 was. **Cost, recorded in the docs rather than discovered later:** §1.1 of the smoke doc — the **`switched stack min_rsp=… limit=… size=…`** measurement — and the **`before py_install_idt`** confirmation both need a throwaway rebuild with the flag now, so **§1.1 became a diagnostic procedure instead of a per-build step**. **Upside beyond a quiet console:** the diagnostic asymmetry between the toolchains is gone, and by *removal* rather than by adding the flag to GCC — a stock image of either toolchain behaves and reports identically.
34. **2026-09-09 — §11.8 #1 CLOSED on GCC hardware; the latent-defect list is now EMPTY.** GCC rebuilt with the unified alignment and verified through the interactive REPL: **`Python312.efi`** → **`import json`** → **`exit()`** → Shell **`exit`**, all clean, **no hang**. That is the correct test rather than a cheap one: the entire risk was whether GCC still runs on a stack base rounded **0–511 bytes higher**, and a deep import at the **`>>>`** prompt is the deepest stack use this port has available. MSVC needed no run — it already compiled the surviving expression, so only comments changed on its side. **All four defects recorded in §11.8 on 2026-09-08 are now fixed and verified on both toolchains:** #1 alignment (today), #2 **`edk2_alloc_environ()`** leak, #3 MSVC IDT, #4 GCC silent fault trap. **What that leaves:** the two toolchains now share the entry path, the stack switch, the alignment expression, the IDT, fault reporting *and* — since the boot traces came off (item 33) — their diagnostics. The remaining differences are **`UEFI_C_SOURCE`**-gated (`Py_FinalizeEx()` detach) rather than entry-path behaviour. **Scope of this run, stated precisely:** the GCC image tested here **predates** item 33 — it still printed **`Python312: UefiMain`**, which is the discriminator — so it verifies the **alignment change only**. The boot-trace removal is **unverified on either toolchain** as of this entry.
35. **2026-09-09 — the traces are actually off, and GCC confirms it on hardware. Item 33's INF edit was necessary but not sufficient — two further things kept output alive, and hardware found them one per attempt.** **(a) A DSC-level define outranked the INFs.** **`AppPkg.dsc`** carried a **`!if $(BUILD_PYTHON312)`** / **`[BuildOptions]`** block adding **`/DPY_UEFI_BOOT_TRACE=1`** to **every** MSFT module in the package, Python312 included, so removing it from the two INFs changed nothing on VS2022 — the flag was still defined. Now commented out, with the reason recorded there; **re-enabling traces means uncommenting the INF line *and* the DSC block**, which the INF comments now say. **(b) A second unguarded print, in a different idiom.** **`python.c:35`** emitted **`Python312: enter main`** with **`fputs(…, stdout)`**, not firmware **`Print(L"…")`** — a narrow string down the stdio route. Item 33's sweep looked for **`Print(L`** and so could not have found it; the lesson is to search by **effect** (any output call) rather than by the one API the known instance used. Gated now, together with the **`fflush`** that existed only to push that line out. **The sweep was then done exhaustively rather than waiting for hardware to find a third:** **`python.c:35`** was the *only* ungated output under **`Programs/`**, **`Modules/main.c`** has none, and **`StdLib/LibC/Main/Main.c`** is entirely behind **`STDLIB_BOOT_PRINT`** apart from its genuine **`ERROR Initializing Standard IO`** path, which stays. **GCC verified green:** rebuilt image prints **nothing** on a normal boot, and **`import json`** plus the teardown route (REPL exit → Shell **`exit`**) is clean, **no hang**. **GCC is the toolchain that proves the gating**, which is why this order was worth having: it never had **`PY_UEFI_BOOT_TRACE`** defined, so those two ungated lines *were* its entire console — a silent GCC image demonstrates both gates directly, with no flag state to confound it. **VS2022 FULL then confirmed the same day, closing the item:** rebuilt, both strings **absent** from the image on the two-encoding check — **`UefiMain`** wide from **`Print(L)`**, **`enter main`** ASCII from **`fputs`**, a mismatch that itself masked (b) for one round — and **`-S -c "print(1+1)"`** on hardware emitted **`2`** and nothing else. That is the toolchain the DSC define was defeating, so it is the one that verifies (a). **Scope, stated rather than assumed:** the two **MIN** images were **not** rebuilt for this. Silence follows there by construction rather than by test — MIN compiles the *same* **`edk2main.c`** and **`python.c`** off the *same* **`MSFT:`** **`CC_FLAGS`**, and **`Python312_MIN.inf`** lost the define in item 33 — so there is no mechanism by which a MIN image could still print a boot line once both gates are in place and no define reaches them. **Both toolchains now ship a console that carries only the program's own output**, plus the error and CPU-fault paths that stay unconditional by design.
36. **2026-09-09 — CPU faults are now SURVIVABLE, verified on hardware: `uefi.mem_read` / `mem_write` / `mem_probe` and `uefi.FaultError` land at `bf92e8a3`, and VS2022 MIN passes §5.9 tests 0–8.** The `edk2_seh_*` setjmp/longjmp path in `edk2excep.c` has been complete but **callerless for the entire port** — `g_context_index` never left `-1`, so its recovery branch was dead and every fault took report-and-spin. These three primitives are its first caller, and the run makes the mechanism **known to work rather than merely written**: a #GP on `0x800000000000` raises `FaultError` (`vector == 13`, with `rip`/`cr2`/`error_code`), **the prompt comes back**, and a subsequent good read still works — which is the row that distinguishes real recovery from an interpreter that merely looks alive. **The scope is narrow on purpose and that is the design, not a limitation:** the guarded region is one leaf load or store, because recovery discards the intervening C frames with no unwinding — no `DECREF`, no allocator or container repair — so guarding anything holding a Python reference would corrupt the interpreter instead of reporting (design §3 rejects `seh_call(fn, …)` for exactly this). **Two earlier decisions paid off here, both confirmed by observation rather than argument.** `time.sleep(2)` returning in ~2 s proves the §2.1 fix: the `longjmp` skips the `iretq` that restores `RFLAGS`, and without the conditional `EnableInterrupts()` the firmware timer would have stayed masked — a failure that would have surfaced later as unrelated-looking console misbehaviour. 200 consecutive recovered faults leaving the guard stack balanced exercises §2.2, whose absence would have turned into out-of-bounds writes rather than a clean `RuntimeError`. **The configuration choice was deliberate:** MIN needs no `ctypes` or OpenSSL, builds fastest, and §5.9 needs nothing but the builtin `uefi` module — *and* MIN is where fault routing was recorded in item 30 as **unobservable**, since §5.8 requires `_ctypes` to dereference an address. A `FaultError` with the right vector is direct proof the IDT entry is live there, so **the one gap item 30 called unclosable is closed**. Also fixed on the way: `edk2excep.h` never included `Protocol/DebugSupport.h` for the two types it uses, compiling only because both existing includers pulled in `Protocol/Cpu.h` first. **Remaining: VS2022 FULL, GCC FULL and GCC MIN** — and GCC test 0 doubles as the answer to whether `UEFI_C_SOURCE` really is `MSFT:`-only, a claim the tree contradicts (see design §4).
37. **2026-09-09 — VS2022 FULL green too, including the one thing MIN could not test: a *successful* `mem_write`.** §5.9 tests 0–8, the `ctypes` write path and the §2/§3/§4 regression all pass ([`Python312_Smoke_Tests.md`](./Python312_Smoke_Tests.md) §7.6). **The write test is the only row that proves a *good* address still does the ordinary thing** — every other row proves the guard survives a *bad* one — and it is read back through `ctypes`' own view of the buffer, so the byte genuinely reached memory rather than the call merely returning without error. It needs `_ctypes`, which is why it is FULL-only and why MIN's sign-off could not include it. **Repeating §5.9 on FULL was not redundant, for a reason worth recording:** the primitives are identical code, so FULL cannot change their behaviour — it changes their *surroundings*, linking OpenSSL, zlib and `_ctypes`, which moves where a fault lands relative to the rest of the image and adds far more state for a non-unwinding `longjmp` to disturb. Green on both MIN and FULL therefore says the mechanism does not depend on how much else is loaded, which is a stronger claim than either run alone. **Two of four configurations left, both GCC**, and the GCC run carries the extra question: test 0 there is also the experiment that settles whether `UEFI_C_SOURCE` is really `MSFT:`-only.
38. **2026-09-09 — GCC FULL green, and it disproved a documented deviation on the way: `UEFI_C_SOURCE` is NOT `MSFT:`-only.** §5.9 tests 0–8 plus the `ctypes` write row all pass on GCC FULL ([`Python312_Smoke_Tests.md`](./Python312_Smoke_Tests.md) §7.7), banner `[GCC 5.3.1 20160413] on uefi`. **Every number matches VS2022 except `rip`**, which differs only because the images do. That parity is the expected result for a specific reason and this run is what confirms it: `setjmp`/`longjmp` here is **EDK2's `SetJump`/`LongJump`**, the same assembly on both toolchains, with no SEH-based unwinding on either — so there was no toolchain-specific mechanism left to diverge, unlike the NASM ABI and `rsp`-relative frame problems that made the stack switch MSVC-only for weeks. **The incidental finding is the more valuable one.** Test 0 returning `True` on GCC is impossible if `UEFI_C_SOURCE` is `MSFT:`-only, since `PyInit_uefi` and `mem_read` both sit behind that gate — and tracing it produced the mechanism: a **package-wide `[BuildOptions]` block in `StdLib/StdLib.inc`** (`GCC:*_*_*_CC_FLAGS = -nostdinc -nostdlib -DUEFI_C_SOURCE`, line 125; `MSFT:` the same at 124) that **`AppPkg.dsc:159` `!include`s**, reaching every module in the package. **Three claims across the docs were wrong and are now corrected** (smoke §7, WSL build guide, deviations §2.3): the `/DUEFI_C_SOURCE` on the MSFT INF line is **redundant**, **every `#ifdef UEFI_C_SOURCE` block is live on both toolchains** so none of them is an MSFT-only deviation, and specifically **the `Py_FinalizeEx()` detach is compiled into GCC images too** — the smoke doc had asserted it was VS2022-only. **The generalisable lesson, and this is the second instance:** a DSC-level `[BuildOptions]` entry invalidated a conclusion drawn from reading the INFs alone, exactly as `PY_UEFI_BOOT_TRACE` did in item 35. When asking whether a define reaches a module, `AppPkg.dsc` **and everything it `!include`s** count as much as the INF. **One configuration left: GCC MIN.**
39. **2026-09-09 — GCC MIN compile-verified and the guarded primitives signed off. Tag `python312-survivable-cpu-faults-2026-09-09`.** GCC MIN **builds clean** and was **deliberately not swept** ([`Python312_Smoke_Tests.md`](./Python312_Smoke_Tests.md) §7.8), which is a judgement worth recording because this project has been burned by unexercised configurations before (items 29 and 30). **The difference from those cases:** they were configurations carrying *accumulated deltas that had never run anywhere*; this is one cell of a 2×2 whose other three cells are observed. GCC FULL against VS2022 FULL varied the **toolchain** and produced identical numbers; VS2022 MIN against VS2022 FULL varied the **module set** and did likewise. GCC MIN is the intersection of two axes each already varied against the other, and for it to fail alone something in the primitives would have to depend on a FULL-only define or symbol — nothing does, which the **build** is precisely what confirms (`PyInit_uefi` resolves; no FULL-only symbol under the GCC linker). **What stays inferred, said plainly:** fault *routing* on that specific image, because MIN has no `_ctypes` and so §5.9 is its only fault test. Two lines (§5.9 tests 0 and 1) close it whenever a GCC MIN image is next on hardware. **State of the capability: three configurations observed, one compile-verified**, and the tag says exactly that rather than implying four sweeps. **What this milestone actually is:** the port went from *reporting* CPU faults and spinning forever to *surviving* them at a Python-visible level, on both toolchains, with the narrow-by-design surface (`uefi.mem_read` / `mem_write` / `mem_probe`) that §3 of the design doc argued for over guarding arbitrary callables.
40. **2026-09-09 — CHIPSEC platform API gap analysed; the answer is 19 functions, not a module.** [`Python312_Chipsec_Platform_API_Port.md`](./Python312_Chipsec_Platform_API_Port.md) records it. `edk2module.c` is two modules welded together: an `os` implementation (~250 entries) that needs **nothing** carried forward because upstream 3.12's `posixmodule.c` is a strict superset of it, and **19 platform functions** at `edk2module.c:4896-4914` (`rdmsr`, `cpuid`, `readpci`, `readmem`, `swsmi`, `GetVariable`, …) that have no equivalent in 3.12.13. Only the second half is the gap. **Three findings changed the shape of the work.** First, `cpu.nasm` — the assembly `swsmi` needs — is **already compiled into both INFs on both toolchains** (`Python312.inf:1058`, `Python312_MIN.inf:317`) and **nothing calls it**: dead code in every image we ship, the same pattern `edk2_seh_*` was in two weeks ago, and already MS-x64-ABI-correct so it needs none of the NASM fixes `edk2stack.nasm` did. Second, `IoLib` and `PciLib` already resolve in `AppPkg.dsc:69-70`, so the PCI and port-I/O APIs need an INF line, not a DSC change. Third and most consequential: **the guarded primitives from item 39 are exactly what this port needs.** 3.6.8's `readmem_dword` dereferences a caller-supplied address raw, which before this month was a silent hang — layering those four functions on `uefi_guarded_access()` makes the most dangerous part of the CHIPSEC surface raise `uefi.FaultError` instead, so the port is strictly better than the thing it replaces rather than a transcription of it. **Two blockers that are not function count.** The module was renamed `edk2` → `uefi` (`posixmodule.c:538`), so CHIPSEC's `import edk2` fails even after a complete port; a `Lib/edk2.py` shim is the low-risk fix. And 3.6.8's init **hard-fails the whole module** if MP Services is absent (`edk2module.c:5142-5150`, `return NULL` before `PyModule_Create`) — tolerable when the module was one import among many, interpreter-bricking now that `uefi` is what `importlib._bootstrap_external` loads at startup, so MP Services must be located lazily inside the four `_ex` functions or not at all. Also documented: `allocphysmem` truncates its pointer to 32 bits via `Py_BuildValue("(I)", …)` with the correct `"(K)"` line sitting commented out beside it. **No code written; scope now fixed.** All three gating decisions were taken the same day (§9): **all 19 APIs**, **FULL only** (`Python312.inf`; MIN untouched), and **naming shim option 1**. Two of those cut the other way from the original recommendation, which is worth recording. Porting all 19 means the three `_ex` variants stay in, so **MP Services is a hard dependency** and §6.1's lazy location is no longer advice but a requirement — it is the one part of this port that can prevent the interpreter from starting, which is why it is sequenced last, after everything else is verified. It also means `allocphysmem` has to be *rebuilt* on `gBS->AllocatePages`/`AllocateMaxAddress` rather than skipped, since the 3.6.8 version does not do what its name claims. The naming-shim decision recorded here was **superseded within the hour — see item 41**, which replaced the whole approach it belonged to. **FULL-only halves the verification matrix** to VS2022 FULL and GCC FULL, versus the 2×2 items 36-39 needed.
41. **2026-09-09 — the CHIPSEC APIs get a module of their own, and that retires the shim, the startup risk and the biggest objection at once.** User proposal, adopted: put all 19 platform functions in a **separate non-bootstrap builtin module** with its own `PyMod-3.12.13/Modules/edk2module.c`, using `uefi` internally as needed — rather than extending `posixmodule.c` as item 40 planned. **This is a better design than what I wrote, and it is worth being precise about why.** The dominant reason is the startup path: `uefi` *is* the `os` module and `importlib._bootstrap_external` imports it while the interpreter is coming up, so anything added to `posixmodule.c` executes during startup, where a failed `AddObjectRef` or a mistimed protocol lookup does not disable a function — it stops Python from starting, on a platform where there is then no Python to debug it with. `_PyImport_Inittab` (`PyMod-3.12.13/Modules/config.c:111`) is a **static table scanned only on import**, so a new entry costs nothing at boot and *cannot* affect startup; the worst failure becomes `import edk2` raising with a readable traceback. After the month this project spent chasing pre-`main` hangs through boot traces, moving 19 privileged functions off that path is worth far more than a file split costs. **Three further wins fall out for free.** The naming problem disappears rather than getting solved: name the module `edk2` and CHIPSEC — and the second tool — import it unchanged, so **item 40's `Lib/edk2.py` shim is withdrawn entirely**, along with its filesystem-`Lib` dependency and its inability to be tested on a bare `.efi`. Registration is one extern plus one inittab line, following the `edk2console` precedent at `config.c:204`. And nothing of ours lands in the ~17.5k lines of upstream `posixmodule.c`, which is permanent merge friction we were about to take on for no reason. The right framing for the docs: 3.6.8's `edk2` module is being **split in two** — the `os` half already moved and became `uefi`, the platform half keeps the name `edk2`. **One real obligation comes with it.** The fault-recovery path must be *shared*, not copied, or one of the two copies silently rots: `uefi_guarded_access` (`posixmodule.c:16056`) is pure C with no Python in it and moves to `efi/src/edk2excep.c`, `uefi_set_fault_error` is generalised to take the exception type rather than the module, and the new module **reuses `uefi.FaultError` as the same object** so `except uefi.FaultError` keeps catching everything. Because that is the one verified-on-four-configurations path in the tree, phase 2 of the plan is the refactor alone, with §5.9 re-run on both FULL configs to show it changed nothing. **Also confirmed:** the `_ex` variants are needed — by a second tool, not CHIPSEC — so MP Services stays a hard dependency. The separate module downgrades §6.1 from interpreter-bricking to import-failing, but the lazy-location requirement stands on its own merits: 16 of the 19 functions have nothing to do with MP Services, and locating it at init would trade those 16 for 3. Plan rewritten to 9 phases; scope otherwise unchanged from item 40.
42. **2026-09-09 — phase 1 of the CHIPSEC port written: the `edk2` module exists and does nothing. Untested.** Three files: new `PyMod-3.12.13/Modules/edk2module.c` (empty method table, single-phase init following `edk2console.c`), `PyMod-3.12.13/Modules/config.c` (extern plus `{"edk2", PyInit_edk2}`), and the `Python312.inf` `[Sources]` line. **The gating mechanism already existed and is reused rather than invented:** `config.c` is shared between the FULL and MIN INFs, and it already brackets `zlib`/`_ctypes`/`_hashlib`/`_ssl` in `#if defined(BUILD_PYTHON312_FULL)` — which is a real C define on both toolchains in `Python312.inf:1087-1088` and only a *comment* in `Python312_MIN.inf:2`. Both new lines go inside those existing blocks, so FULL-only needs no new machinery. `srcprep.py` needs no change either: the INF compiles the PyMod path in place, as it already does for `posixmodule.c`. **Acceptance is written up as §10 of the port doc, and two of its tests are worth calling out.** MIN must be *built* even though it gains nothing — a shared `config.c` means a wrong gate on either the extern or the inittab entry shows up as MIN failing to link on `PyInit_edk2`, so a clean MIN link is the test of the gate and costs one build. And `import sys; print(len(sys.modules))` must come back **unchanged from the pre-change image**, which is the direct evidence for the whole non-bootstrap argument in item 41: if `edk2` were being dragged in at startup the count would rise by one, and if it is genuinely inert it is identical. That wants a same-day before/after on the deployed image rather than trusting the historically observed 23, since the count drifts as modules land. On MIN the pass condition is `ModuleNotFoundError`. **Deliberately tests nothing about MSR, PCI, memory or the fault path** — there is no code for any of it yet, and the point of a separate phase is that when phase 3 misbehaves the module plumbing is already known good.
43. **2026-09-09 — phase 1 green on VS2022 FULL: `import edk2` works and the module is provably inert at startup.** All of §10.2 observed as written, teardown included, no hang. **The acceptance test was replaced mid-flight with a better one, which is the part worth recording.** The original design asked for `len(sys.modules)` before and after the change, reasoning that the count would rise by one if `edk2` were being pulled in during startup. That is a proxy dressed up as a proof: it needs a same-day baseline captured from the outgoing image *before* it gets overwritten, and the count drifts as unrelated modules land, so a mismatch would have been ambiguous rather than informative. `'edk2' in sys.modules` → **`False`** measures the thing itself and needs no baseline at all; paired with `'edk2' in sys.builtin_module_names` → **`True`** it states the property directly as **registered but not loaded**. Both landed as expected, so the central claim of item 41 — that an inittab entry is a static table scanned only on import and therefore costs nothing at boot — is now **measured rather than argued**. `print(1+1)` still emits `2` and nothing else, so the new translation unit reintroduced no boot chatter. **What this buys the remaining phases:** the module plumbing is known good in isolation, so from phase 3 onward a failure is about the function just added rather than about registration, gating or the inittab — which is the entire reason for spending a build on a module that does nothing. **Still outstanding:** the MIN half of the gate check (clean link, and `import edk2` raising `ModuleNotFoundError`). MIN cannot regress functionally here since it compiles neither the new file nor its inittab entry, but a clean MIN link is the only thing that actually proves the `BUILD_PYTHON312_FULL` bracket around the shared `config.c` is correct, so it is not skippable.
44. **2026-09-09 — phase 1 closed (MIN links clean), phase 2 written: the fault path is now shared rather than duplicated.** MIN **built and linked clean**, which closes the gate question: `config.c` is shared between both INFs, so a wrong `BUILD_PYTHON312_FULL` bracket would have surfaced as an unresolved `PyInit_edk2` at MIN link time. The runtime `ModuleNotFoundError` check was skipped deliberately, not forgotten — it costs an image swap to learn something the link already proved. **Phase 2 is four files and adds no capability.** `uefi_guarded_access` moved **whole** out of `posixmodule.c` into `efi/src/edk2excep.c` as `edk2_guarded_access()`, body untouched, declared in `edk2excep.h`; the three `mem_read`/`mem_write`/`mem_probe` call sites retarget to it; and `uefi_set_fault_error` now takes the **exception type** rather than the module, so a caller with no posix state can use it. `uefi_set_fault_error` stays `static` on purpose — generalising the signature is the enabling step, but choosing *how* to export it would be speculative until something else raises a fault, which is phase 5's problem. **Two things were verified before the move, not after**, and are recorded in §5.2 so nobody re-derives them: `setjmp` resolves to the same EDK2 `SetJump`/`LongJump` in the destination, because `edk2excep.c:1` already includes `<setjmp.h>` and `posixmodule.c` only ever saw `jmp_buf` through `edk2excep.h`; and the function had to move **whole**, since `setjmp` must be called by the frame `longjmp` returns to — factoring out an "arm the guard" helper would compile, read fine, and arm the guard against a frame that had already returned. Parameter types stay spelled `unsigned long long` rather than `uint64_t` to match the callers' `PyArg_ParseTuple("K")` values, since the two spellings need not be the same type under GCC and a `volatile unsigned long long *` would then not be assignable to a `volatile uint64_t *`. **The phase gets one positive test rather than only "nothing broke":** `PyInit_edk2` now binds `edk2.FaultError` to `uefi.FaultError` — the same object — so `edk2.FaultError is uefi.FaultError` must print `True`. `False` would be the nastier outcome of the two possible failures, since code catching `edk2.FaultError` would still work while code catching `uefi.FaultError` would silently stop catching anything. **Acceptance (§11):** all four configurations must build, because unlike phase 1 this touches code MIN compiles; and §5.9 must be re-run on VS2022 FULL and GCC FULL with every value matching §7.5-§7.7 apart from `rip`. That path is verified on four configurations and the code underneath it just changed translation units, so any difference is a regression rather than a curiosity. **Also settled a non-issue:** em dashes in the new C comments are fine — `edk2main.c:217`, `environ.c:32` and `pyconfig.h:1803` have carried them on both toolchains for weeks, and phase 1's own `edk2module.c:1` built with one.
45. **2026-09-09 — phase 2 green on VS2022 FULL: the guarded path changed translation units and nothing noticed.** §5.9 tests 0–8, the `ctypes` write row (`b'A'`) and the §2/§3/§4 regression all pass on an image where `uefi_guarded_access` no longer lives in `posixmodule.c` ([`Python312_Smoke_Tests.md`](./Python312_Smoke_Tests.md) §7.9). Plus the phase's one positive assertion: **`edk2.FaultError is uefi.FaultError` → `True`**, so the borrow works and there is a single exception type rather than two that can drift. **This re-run was designed to detect nothing, which is what makes the individual passing rows worth naming** — four of them fail in distinct, recognisable ways, and between them they cover everything a bad move could have done: row 1 (fault raises *and the prompt returns*) would catch the relocated `setjmp` no longer owning the frame `longjmp` returns to; row 3 (a read after the fault) would catch recovery that merely looks successful while leaving the interpreter subtly unusable; row 4 (the `2.0` sleep) would catch `RFLAGS.IF` not being restored, i.e. the design doc §2.1 defect regressing; row 6 (200 faults, then a read) would catch a leaking guard stack, which shows up as `edk2_seh_try()` returning `NULL`. All four green. **§5.8 was skipped on purpose and the reasoning is worth keeping:** it injects an *unhandled* fault, costs a power cycle, and exercises the branch of `py_handle_exception` that phase 2 never touched — row 1 covers the handled branch, which is the part that actually moved. Recommending it earlier was wrong. **The `edk2` module surface was then checked properly, because identity alone was not enough.** `edk2.FaultError is uefi.FaultError` says the two names reach one object; it says nothing about whether that object catches a fault raised through the *other* module, which is precisely what a consumer writing `except edk2.FaultError` around the memory APIs depends on. `isinstance(sys.last_value, edk2.FaultError)` after an unhandled `uefi.mem_read` → **`True`** closes that, with no `try`/`except` needed thanks to the `sys.last_value` convention §5.9 already uses. `issubclass(edk2.FaultError, OSError)` → **`True`** covers a different audience again: tooling that catches `OSError` broadly and knows nothing about this build still catches a fault, which is the reason `OSError` is the base. Worth recording that the traceback **correctly** still reads `uefi.FaultError` — one type keeping the name it was created with, exposed under two module attributes — so that nobody later files it as a defect. Surface inventory confirmed `['FaultError']`, and that check is now standing for every remaining phase: it is the cheapest guard against a function added to the method table but misspelled, or written and never added at all, both of which raise `AttributeError` on the name you expected and neither of which implicates the table. **Outstanding before phase 2 closes:** VS2022 MIN and GCC MIN **builds** (compile only — both INFs compile `posixmodule.c` and `edk2excep.c`, so neither may break, though neither gains anything), and GCC FULL build plus a §5.9 re-run. Unlike phase 1, this phase touches code MIN compiles, so a MIN build is not optional this time.
46. **2026-09-09 — phase 2 build matrix complete; phase 3 written.** VS2022 MIN, GCC MIN and GCC FULL all **build clean** at the phase 2 code state, which was the outstanding item from 45. The MIN builds were the ones with something to prove: both INFs compile `posixmodule.c` and `edk2excep.c`, so the move of `uefi_guarded_access` between those two translation units had to survive a configuration that does **not** compile `edk2module.c` — a declaration that only worked because of an include FULL happens to pull in would have surfaced here. GCC FULL §5.9 re-run is with the user; the binary is already built, so it is independent of tree state from this point. **Phase 3 is written on top of it:** `rdmsr`, `wrmsr`, `cpuid`, `readio`, `writeio` in `edk2module.c`, plus `IoLib` in `Python312.inf`'s `[LibraryClasses]` — `BaseLib` deliberately not added, since it is already in `UefiLib`'s closure, which is what lets `edk2excep.c` call `EnableInterrupts()` today. **Reading 3.6.8's port I/O closely turned up a defect worse than the one this port set out to avoid.** `readio` did `addrs = (short)(addr & 0xffff)` — the mask reads as a range guard, but the cast is to a **signed** short, so every port from 0x8000 up became negative and sign-extended inside `IoRead8(UINTN)`: the upper half of the I/O space silently addressed something else. Worse, there was no alignment check, and MdePkg's `IoRead16`/`IoRead32` `ASSERT` on a misaligned port while this build ships `PcdDebugPropertyMask` at 0x0F — so `readio(0x81, 2)` on 3.6.8 does not return a wrong value, it **stops the machine in `DebugAssert()`**. Both are now `ValueError`, and the `& 0xFF` truncation on writes is now `OverflowError`, matching the call `uefi.mem_write` already made. These are behaviour changes, recorded as such in the port doc §12.1 rather than as cleanups. **The acceptance in §12 is built around not trusting this document.** CPUID leaf 0 returns the vendor string across ebx/edx/ecx, so a correct result is the readable text `b'GenuineIntel'` and any error in argument passing or register order produces garbage rather than a plausible number; `rdmsr(0x1B)` (IA32_APIC_BASE) was picked over an arbitrary MSR because its value has **checkable structure** — 0xFEE00000 base, enable bit 11, BSP bit 8 — so a wrong answer looks wrong. Best of the three: `readio`/`writeio` are validated by driving 0xCF8/0xCFC as a PCI config read of 00:00.0 and comparing against the Shell's own `pci 00 00 00`, which checks port, width and value at once against a source that is not me. `wrmsr` gets **no default test** and that is deliberate: no MSR is safe to write on an arbitrary platform, its three lines share `rdmsr`'s plumbing, and the optional TSC_AUX round-trip in §12.7 is gated on a CPUID check for RDTSCP because writing a non-existent MSR raises #GP — which, unlike a bad memory access, is **not** survivable. The phase-2 guarded path covers memory only; MSR and port access have no fault protection, and §12 says so in place rather than leaving it to be discovered.
47. **2026-09-09 — CHIPSEC port phase 2 CLOSED: GCC FULL matches VS2022 FULL row for row.** §5.9 tests 0–8, the `ctypes` write row and the three `edk2` module checks all green on GCC FULL ([`Python312_Smoke_Tests.md`](./Python312_Smoke_Tests.md) §7.10), so the shared guarded fault path is now verified after the move on **both** FULL toolchains and compiled clean in both MINs. **This was the only run in the phase that could have failed, and that is worth stating plainly rather than logging a fourth green column.** The two MIN builds are compile-only, and the VS2022 FULL re-run used the same compiler that had already been proven against the code in its old location — neither varied anything with a mechanism for breaking. Moving `uefi_guarded_access` into `edk2excep.c` changes which translation unit owns the frame that `setjmp` captures and `longjmp` returns to; `SetJump`/`LongJump` is byte-identical EDK2 assembly on both toolchains, but **the frame around it is not** — MSVC and GCC disagree about what lives in registers versus the stack and about what the optimiser may keep live across a call that returns twice. So a relocation defect was structurally more likely to appear on one compiler than on both, which is why GCC agreeing is the result that closes phase 2 rather than a formality. Phases 1 and 2 together mean the `edk2` module exists, is provably inert at startup, and shares one verified fault path and one `FaultError` type with `uefi`. **Everything from phase 3 on adds capability to that frame rather than changing it** — which also means the risk profile inverts: the remaining phases can fail in ways that stop the machine (MSR #GP, misaligned port I/O, MP Services in phase 9), where phases 1 and 2 could only fail by raising.
48. **2026-09-09 — phase 3 green on VS2022 FULL: MSR, CPUID and port I/O live, and the first port phase to add real capability.** All of the port doc §12.2–§12.6 as written, plus a clean teardown. **Three of those rows prove something a green tick does not convey.** `cpuid(0,0)` returning `b'GenuineIntel'` is the ABI check: that string can only assemble if the leaf argument, all four result registers and their order are simultaneously right, so it is precisely the test that would have caught the malformed `(IIII))` `Py_BuildValue` format 3.6.8 shipped. The 0xCF8/0xCFC read **agreed with the Shell's own `pci 00 00 00`**, which confirms port number, access width and data against a witness that is not this project's expectations — worth preferring over an expected constant for anything hardware-facing. And all four argument checks raised *before* touching the port with the prompt still live afterwards, which is the deviation from 3.6.8 earning its keep: `readio(0x81, 2)` there does not return a wrong value, it trips the `ASSERT` in MdePkg's `IoRead16` and stops the box, and this build ships `PcdDebugPropertyMask` at 0x0F so that assert is live. **Two things remain deliberately unproven.** `wrmsr` has no default test because no MSR is safe to write on an arbitrary platform; its three lines share `rdmsr`'s plumbing and §12.7's TSC_AUX round-trip is available, gated on a CPUID check for RDTSCP. And behaviour on a *non-existent* MSR or port is not testable at all, because it is not survivable: #GP from a reserved MSR is taken outside the phase 2 guard, which covers memory accesses only. **GCC FULL for phase 3 was skipped by decision, and the reasoning is checkable rather than optimistic** (port doc §12.8): the main thing a GCC link would have settled — whether `IoLib` and `PciLib` resolve there — is answered statically, because both are in `AppPkg.dsc`'s **common** `[LibraryClasses]` (lines 69-70) with no toolchain conditional, so resolution is identical on both. And unlike phase 2, there is no `setjmp` frame whose layout differs between compilers; the code is straight-line calls into BaseLib and IoLib. What genuinely stays unverified is only that the file *compiles* under GCC, which is a build-time failure that cannot hide — it will surface on the next GCC FULL build for any later phase, so it cannot escape into a shipped image.
49. **2026-09-09 — phase 4 written (`readpci`, `writepci`), and reading MdePkg first turned up a halt-the-machine path that 3.6.8 exposed to Python.** `PciLib` added to `Python312.inf`; no DSC change needed, since `AppPkg.dsc` already maps it to `BasePciLibCf8`. **That mapping is the whole story of the phase.** Every access goes through legacy 0xCF8/0xCFC, and `BasePciCf8Lib` validates addresses with `ASSERT (((A) & (~0xffff0ff | (M))) == 0)` where `M` is 0/1/3 for the 8/16/32-bit accessors — so with `PCI_LIB_ADDRESS` putting the register in bits 0..11, **two distinct arguments reach an ASSERT**, and asserts are live in this build at `PcdDebugPropertyMask` 0x0F. First, any offset >= 0x100: the MdePkg header states it outright ("If the register specified by Address >= 0x100, then ASSERT()"). 3.6.8 took `unsigned char off`, which *accidentally* dodged that by wrapping 0x100 to 0x00 — and then returned register 0x00's contents **as though they were the register asked for**, which is the worse failure of the two. Second, a misaligned offset for the width, which 3.6.8 did nothing about: `readpci(0,0,0,2,4)` there does not return a wrong value, it stops the box. Both are now `ValueError`. Third, out-of-range bus/dev/func, which `PCI_LIB_ADDRESS` masks silently — on 3.6.8 that turned a nonexistent device into a **different, possibly populated** one, harmless for a read and not at all for a write. **Consequence worth escalating to whoever drives CHIPSEC:** extended configuration space (0x100-0xFFF, where PCIe capabilities live) is unreachable in this build, full stop. That is a property of the DSC's `PciLib` choice and not of this code — remapping `PciLib` to an ECAM implementation extends the reach with **zero change to `edk2module.c`** — but anything walking PCIe extended capabilities needs that remap first, and the range check is there so the limit gets reported instead of guessed at. **One thing deliberately not tidied:** `writepci(bus, dev, func, off, value, size)` takes value *before* size, the opposite of `writeio(port, size, value)`. It is inconsistent and it is 3.6.8's published signature; callers pass positionally, so "fixing" it would silently swap two integers at every existing call site. Comment in the source says so. **§13's acceptance leans on cross-checks rather than expected constants:** three widths of the same register check each other arithmetically (2 bytes must be the low half of the 4-byte read), and the 4-byte value must agree with *both* the Shell's `pci` and phase 3's hand-driven CF8 read — three independent paths to one number. `writepci`'s default test writes to a b/d/f that `pci` does not list, so the call path is exercised with nothing listening; the stronger optional test writes the architecturally read-only vendor ID and confirms via read-back that it was ignored, with an explicit stop-and-power-cycle instruction if it was not.
50. **2026-09-10 — phase 4 green on VS2022 FULL, and a false defect report that is worth keeping as a method note.** All of §13.3-§13.7 as written. The read-only vendor-ID test passed incidentally: a write landed on 00:00.0 offset 0 and the register still read `0x8086` afterwards. **The interesting part was the one row that appeared to fail.** `writepci(0,0,0,0,0x10000,2)` was reported as not raising `OverflowError`; the code was correct and the screenshot showed why — the console had dropped a zero, so the call actually made was `0x1000`, which **fits** in two bytes, and not raising was the right answer. Two fixes came out of it. **First, a real documentation defect:** the expected `sorted(dir(edk2))` lists were wrong, because `wrmsr` sorts *after* `writeio` and `writepci` — Python compares character by character and `'i' < 'm'` — while I had grouped the MSR names together on intuition. The source now carries a comment warning against "correcting" the doc to match the method table's ordering. **Second, a testing-method change with general application:** overflow test values must survive a dropped character. `0x100` degrades to `0x10`, which legitimately fits in a byte and so produces a false failure report against working code; `0xFFFF` cannot be reduced to something that fits by deleting any single character. All overflow rows in §12.6 and §13.6 were changed on that basis. Given the console drops characters intermittently on this hardware, choosing test inputs that fail *closed* under corruption is worth doing everywhere, not just here.
51. **2026-09-10 — phase 5 written: the memory APIs, which is what phases 1 and 2 were for.** `readmem`, `readmem_dword`, `writemem`, `writemem_dword`, all on the guarded path, so **a wrong address raises `FaultError` instead of stopping the machine** — in 3.6.8 `readmem` dereferenced a caller-supplied address byte by byte with nothing in the way, and took a page fault, on a box that was being examined precisely because something about it was already suspect. That inversion is the deliverable of the phase; everything else is detail. **One new C primitive:** `edk2_guarded_copy()` in `efi/src/edk2excep.c`, alongside the existing single-access one. It takes **one `setjmp` for the whole block** rather than one per byte, since a fault anywhere fails the call either way and a partially filled buffer is not usable, and it copies byte at a time through `volatile` pointers instead of calling `memcpy`, because `memcpy` is free to use wide or vector moves and a caller reaching into MMIO may be talking to a device that cares about access width. **Three 3.6.8 defects not reproduced:** `readmem`'s `malloc` failure path returned NULL *without setting an exception* (a `SystemError` where `MemoryError` belongs — avoided here by reading straight into the `bytes` object); `int len` meant a negative length ran the copy loop about four billion times **writing** memory (now `Py_ssize_t` and a `ValueError`); and `writemem` parsed `s#`, accepting `str` and writing its UTF-8 encoding, which is a different byte count than the string has characters as soon as one is non-ASCII (now `y#`, bytes only). **One structural change worth flagging:** `uefi_set_fault_error` is no longer `static`, and is declared in a new `Modules/edk2fault.h`. It stays *defined* in `posixmodule.c` deliberately — MIN compiles that file and not `edk2module.c`, yet MIN still has `uefi.mem_read` and still needs the helper, so defining it in the new module would break the MIN link and would point the dependency the wrong way, given `uefi` is imported during startup and must not depend on a module that may be absent. **Consequence for testing:** this is the first phase since 2 to touch code MIN compiles, so both MIN builds are back in scope and §5.9 needs re-running on FULL. **The acceptance (§14) is the strongest of the port so far**, because `ctypes` lets it aim these functions at a buffer whose contents we chose: UEFI is identity-mapped, so `ctypes.addressof(buf)` is a physical address, `readmem` must return exactly the bytes put there, `writemem` is verified independently through `buf.raw`, and `readmem` and `readmem_dword` are checked against *each other* — they reach the same memory through `edk2_guarded_copy` and `edk2_guarded_access` respectively, so agreement tests both primitives with no external reference involved.
...
| **CHIPSEC platform API port** | `AppPkg/Applications/Python/Python312_Chipsec_Platform_API_Port.md` |
