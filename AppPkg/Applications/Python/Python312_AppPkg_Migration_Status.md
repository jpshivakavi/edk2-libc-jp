...
| 8 | Vendored third-party libs (FULL parity) | **Done** — 8.1 zlib, 8.2 readline, 8.5 libffi/ctypes, 8.3–8.4 OpenSSL (`_hashlib`, `_ssl`) |
...
| Omit modules (remaining) | *(none — Phase 8 vendored ext enabled in `config.c`)* |
| Vendored built-ins | `zlib`, `readline`, `_ctypes` / `_ctypes_test`, `_hashlib`, `_ssl` |
...
### 2026-07-17 — Session 10 (Phase 8.4 OpenSSL `_ssl`)

1. Vendored **`ssl/`** libssl + `Modules/_ssl.c` + `config.c` (commit **`89e208b8`**; see [`Python312_Phase8_8.4_Ssl.md`](./Python312_Phase8_8.4_Ssl.md)).
...
| `python312-apppkg-8.4` | `89e208b8` | **8.4** libssl + `_ssl`; UEFI `import ssl` / default context smoke |

Checkout: `git fetch origin tag python312-apppkg-8.4 && git checkout python312-apppkg-8.4`
...
| 8.4 | libssl + `Modules/_ssl.c` | **Done** — GCC/NOOPT/X64; UEFI `import ssl`, `create_default_context()` (OpenSSL **1.1.1f** on UEFI) |
